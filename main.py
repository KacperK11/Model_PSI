...
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt # Dodano dla pewności
...
for name, model in models_to_test:
    # Tworzymy tymczasowy pipeline dla każdego modelu
    ...
    # SVM i LogReg mogą wymagać probability=True dla predict_proba
    try:
        y_prob_temp = temp_pipeline.predict_proba(X_test)[:, 1]
        roc_auc_val = roc_auc_score(y_test, y_prob_temp)
    except:
        roc_auc_val = 0.5 # Fallback jeśli model nie obsługuje prawdopodobieństw
    
    # Wypisanie wyników w formacie gotowym do przepisania
    print(f"--- WYNIKI: {name} ---")
...
grid_search_recall = GridSearchCV(
    pipeline,              # <-- TEN pipeline już masz
    param_grid_recall,
    cv=5,
    scoring="recall",
    n_jobs=-1,
...
# ============================================================
# SEKCJA: OCENA JAKOŚCI MODELU I ANALIZA PROGU DECYZYJNEGO
#
# W tej sekcji przeprowadzono wizualną ocenę jakości najlepszego
# modelu po eksperymentach naprawczych (GridSearch z optymalizacją
# pod kątem miary recall).
#
# Analiza obejmuje:
# 1) Krzywą ROC oraz wartość ROC AUC – ocenę zdolności modelu
#    do rozróżniania pacjentów z udarem i bez udaru niezależnie
#    od progu decyzyjnego.
# 2) Krzywą Precision–Recall – kluczową dla niezbalansowanych
#    zbiorów danych, ilustrującą kompromis pomiędzy precyzją
#    a czułością klasy pozytywnej.
# 3) Macierze pomyłek dla różnych progów decyzyjnych, pokazujące
#    wpływ zmiany progu na liczbę fałszywie pozytywnych oraz
#    fałszywie negatywnych decyzji.
# ============================================================
...
# Pobierz prawdopodobieństwa dla klasy pozytywnej (udar)
...
for i, thresh in enumerate(thresholds_to_show):
    # Klasyfikacja przy użyciu niestandardowego progu decyzyjnego
...
# ============================================================
# ROZSZERZENIE PROJEKTU: ANALIZA INTERPRETOWALNOŚCI I JAKOŚCI
# ============================================================
# W tej sekcji przeprowadzono dodatkowe eksperymenty mające na celu:
# 1) interpretację działania modelu (feature importance),
# 2) analizę wpływu progu decyzyjnego na jakość predykcji,
# 3) ocenę kosztów błędów klasyfikacyjnych w kontekście medycznym.
# ============================================================
...
#
# Celem tej sekcji jest interpretacja działania najlepszego
# modelu (Random Forest) poprzez analizę ważności cech.
#
# Random Forest umożliwia oszacowanie, które zmienne wejściowe
# miały największy wpływ na podejmowane decyzje klasyfikacyjne.
# Jest to szczególnie istotne w zastosowaniach medycznych,
# gdzie interpretowalność modelu ma kluczowe znaczenie.
# ============================================================
...
# z metody get_feature_names_out(), aby uzyskać pełną listę
# cech wejściowych po preprocessingu.
# ------------------------------------------------------------
...
# Sortowanie i wybór 10 najważniejszych cech
...
# Sprawdzenie czy są braki gdziekolwiek indziej
